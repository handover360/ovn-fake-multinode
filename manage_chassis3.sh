...
if [ ! -f "$LOG_FILE" ]; then
    ...
    for i in {1..3}; do
        # cN_nb_cpu/mem: NB DB 프로세스, cN_sb_cpu/mem: SB DB 프로세스, cN_nr_cpu/mem: Northd 프로세스
        HEADER+=",c${i}_cpu,c${i}_mem,c${i}_disk,c${i}_nb_cpu,c${i}_nb_mem,c${i}_sb_cpu,c${i}_sb_mem,c${i}_nr_cpu,c${i}_nr_mem"
    done
...
collect_stats_csv() {
    ...
    for i in {1..3}; do
        ...
        local c_disk=$(podman exec $node df -m /etc/openvswitch | tail -1 | awk '{print $3}')
        ...
        row+=",$c_cpu,$c_mem,$c_disk,$nb_stats,$sb_stats,$nr_stats"
    done
...
for i in $(seq "$START" "$END"); do
    ...
    if [ "$ACTION" == "add" ]; then
        # 컨테이너 생성 로그가 CSV를 방해하지 않도록 /dev/null 처리
